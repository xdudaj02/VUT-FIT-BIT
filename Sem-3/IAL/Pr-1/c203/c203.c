...
*/
    //loop over all elements of the array an initialize to '*'
...
*/
    //increment index number in range <0, QUEUE_SIZE>
...
*/
    //return true if queue is empty (f_index and b_index point to the same index), false if not empty
...
*/
    //return true if queue is full (all but one indexes are full => b_index + 1 = f_index), false if not full
...
*/
    //if queue is empty call queueError with parameter QERR_FRONT
    if (queueEmpty(q)){
        queueError(QERR_FRONT);
    }
    //else assign value of the first index to parameter c
    else {
        *c = q->arr[q->f_index];
    }
}
...
*/
    //if queue empty call  queueError with parameter QERR_REMOVE
    if (queueEmpty(q)){
        queueError(QERR_REMOVE);
    }
    //else move the 'first index' indicator to next index (remove first character in the queue)
    else {
...
*/
    //if queue empty call queueError with parameter QERR_GET
    if (queueEmpty(q)){
        queueError(QERR_GET);
    }
    //else call queueFront (get first character in queue and assign to parameter c)
    //and then call queueRemove (remove first character in the queue)
    else {
        queueFront(q, c);
        queueRemove(q);
    }
...
*/
    //if queue is full call queueError with parameter QERR_UP
    if (queueFull(q)){
        queueError(QERR_UP);
    }
    //else assign character c to the first empty index and move the 'empty index' indicator to the next index
    else {
        q->arr[q->b_index] = c;
